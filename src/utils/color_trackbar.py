...
def color_trackbar(img_path):
    ...
    cv.createTrackbar('H_h', title_window, slider_max, slider_max,slider_max, t.H_h_change)
    cv.createTrackbar('S_h', title_window, slider_max, slider_max, slider_max,t.S_h_change)
    cv.createTrackbar('V_h', title_window, slider_max, slider_max,slider_max, t.V_h_change)
    while True:
        k = cv.waitKey(0)
        if k == ord('s'):
